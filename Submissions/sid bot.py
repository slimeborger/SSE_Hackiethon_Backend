...
class Script:
    ...
    def get_move(self, player, enemy, player_projectiles, enemy_projectiles):
        ...
        #In Range Of Light & Heavy Attacks
        ...
        def closeRangeTactics():
            enemySpammingBlock = False
            if get_last_move(enemy) == BLOCK and get_past_move(enemy,1) == BLOCK:
                enemySpammingBlock = True
            else:
                enemySpammingBlock = False
                
            
        def threehitcombo():
            if (heavy_on_cooldown(player) == False or primary_on_cooldown(player) == False):
                if get_last_move(player) == LIGHT:
                    if get_past_move(player,2) == LIGHT:
                        if (primary_on_cooldown(player) == False):
                            return PRIMARY
                        return HEAVY
                    else:
                        return LIGHT
